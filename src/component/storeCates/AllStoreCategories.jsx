...
function AllStoreCategories() {
  ...
  const handelDelete = function () {
    setOnload(true);
    fetch(`${route}/store/categories/${delId}`, {
      method: "Delete",
      headers: {
  ...
  useEffect(() => {
    ...
    fetch(`${route}/store/categories?page=${currentPage}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
          ...
          {cates.map((cate) => (
            <tr>
              ...
              <td className="actions">
                ...
                <Link to={`/edit-store-category/${cate._id}`} className="edit">
                  Edit
                </Link>
              </td>
            </tr>
...
export default AllStoreCategories;
